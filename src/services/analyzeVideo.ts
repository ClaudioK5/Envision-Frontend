...
/** Video stats sent to Pulse for per-user cost tracking. */
export type VideoUsageMetadata = {
  size_bytes: number;
...
export type VideoStorageKind = "pa" | "r2";
...
/** Read duration and resolution from a local video file (browser metadata). */
export async function probeVideoMetadata(file: File): Promise<VideoUsageMetadata> {
  const fallback: VideoUsageMetadata = {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: VideoUsageMetadata) => {
    ...
    video.onloadedmetadata = () => {
      ...
      finish({ size_bytes: file.size, duration_sec: duration, width, height });
    };
...
function appendUsageToFormData(
  formData: FormData,
  usage: VideoUsageMetadata,
  storage: VideoStorageKind,
): void {
  formData.append("size_bytes", String(usage.size_bytes));
  if (usage.duration_sec != null && usage.duration_sec > 0) {
    formData.append("duration_sec", String(usage.duration_sec));
  }
  if (usage.width != null && usage.width > 0) {
    formData.append("width", String(usage.width));
  }
  if (usage.height != null && usage.height > 0) {
    formData.append("height", String(usage.height));
  }
...
export async function analyzeVideoStream(
  ...
  if (file.size > PA_MAX_BYTES) {
    ...
    return streamAnalyzeWithObjectKey(
      objectKey,
      trimmedQuestion,
      file.size,
      usage,
